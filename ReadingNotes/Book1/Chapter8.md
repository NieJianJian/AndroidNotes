...
　　只要能被invokestatic和invokespecial指令调用的方法，都能在解析阶段中确定唯一调用版本，符合这个条件的有静态方法、私有方法、实例构造器、父类方法4类，它们在类加载的时候就会把符号引用解析为该方法的直接引用，这些方法称为**非虚方法**，与之相反，其他的方法称为**虚方法**（除去final方法）。final方法虽然使用invokevirtual指令来调用，但是由于无法覆盖，所以**final方法是一种非虚方法**。
