...
　　字段表不会列出从超类或者父接口中继承的字段，但有可能列出原本Java代码之中不存在的字段，譬如在内部类中为了保持对外部类的引用，会自动添加指向外部类的实例字段。
...
　　在Java语言中，要重载一个方法，除了要与原来方法具有相同的简单名称之外，还要求必须拥有一个与原方法不同的特征签名，特征签名是一个方法中各个参数在常量池中的字段符号引用的集合，因为返回值不包含在特征签名中，所以Java语言里是无法仅仅依靠返回值不同来对一个已有方法进行重载的。但在Class文件格式中，只要描述符不是完全一致的两个方法可以共存。也就是说，如果两个方法有相同的名称和特征签名，但返回值不同，也可以共存与同一个Class文件中。
...
　　字节码指令集是一种具有鲜明特点、优势都很突出的指令集架构，由于限制了Java虚拟机操作码的长度为一个字节（0~255），这意味着指令集的操作码总数不可能超过256条。
