...
* 4）虚拟机对对象进行设置。例如这个对象是哪个类的实例、如何能找到类的元数据信息、对象的哈希码、对象的GC分代年龄等信息。这些信息存放在对象头中。

* 5）此时，从虚拟机来看对象已经产生，Java程序来看，对象创建刚刚开始——<init>方法还未执行，所有字段都还为零。一般来说，执行new指令后会接着执行<init>方法，这样真正的对象才算完全产生出来。